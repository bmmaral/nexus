//! Config load order: `--config` > `GITTRIAGE_CONFIG` > `./gittriage.toml` (see `ConfigBundle::load`).
//! Serialized with a global lock because tests mutate `current_dir` and environment variables.

use gittriage_config::{ConfigBundle, ENV_GITTRIAGE_CONFIG};
use std::fs;
...
#[test]
fn explicit_config_path_beats_gittriage_config_env() {
...
"#,
    ...
    let _env = EnvVarGuard::set(ENV_GITTRIAGE_CONFIG, path_b.to_str().unwrap());
...
#[test]
fn gittriage_config_env_loads_when_no_explicit_path() {
    let _lock = SUITE_LOCK.lock().expect("suite lock");
    let dir = tempfile::tempdir().expect("tempdir");
...
"#,
    ...
    let _clear = EnvVarGuard::remove(ENV_GITTRIAGE_CONFIG);
    let _env = EnvVarGuard::set(ENV_GITTRIAGE_CONFIG, path.to_str().unwrap());
...
#[test]
fn cwd_gittriage_toml_used_when_no_explicit_and_no_env() {
    let _lock = SUITE_LOCK.lock().expect("suite lock");
    let _env = EnvVarGuard::remove(ENV_GITTRIAGE_CONFIG);

    let dir = tempfile::tempdir().expect("tempdir");
    let local = dir.path().join("gittriage.toml");
    fs::write(
        &local,
...
"#,
    ...
    assert_eq!(
        ...
        "source path should resolve to local gittriage.toml",
    );
}
