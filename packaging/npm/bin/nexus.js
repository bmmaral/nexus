...
function assetSuffix() {
  ...
  throw new Error(
    `nexus-cli: unsupported platform ${p}-${a}. Use cargo install or see https://github.com/${REPO}#install`,
  );
}
...
async function ensureBinary() {
  ...
  const remoteName = `nexus-v${VERSION}-${suf}`;
  const cacheRoot = path.join(os.homedir(), ".cache", "nexus-cli", VERSION);
  const localName = suf.endsWith(".exe") ? "nexus.exe" : "nexus";
